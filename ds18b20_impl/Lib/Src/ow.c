 ...
 * @param[in]  init: Pointer to initialization data (GPIO, pin, timer,
 * callback).
 */
void ow_init(ow_t *handle, const ow_init_t *init) {
  ...
  HAL_TIM_RegisterCallback(handle->config.tim_handle,
                           HAL_TIM_PERIOD_ELAPSED_CB_ID, init->tim_cb);
...
/*************************************************************************************************/
/**
 * @brief Transfer a command and optional data to/from a specific 1-Wire device
 * by Skip ROM.
 * @param[in] handle Pointer to the 1-Wire handle structure.
 * @param[in] fn_cmd Function or command byte to send.
 * @param[in] w_data Pointer to the data buffer to write (can be NULL if w_len
 * is 0).
 ...
 */
ow_err_t ow_xfer(ow_t *handle, uint8_t fn_cmd, const uint8_t *w_data,
 ...
 * @retval Error code (ow_err_t).
 */
ow_err_t ow_xfer_by_id(ow_t *handle, uint8_t rom_id, uint8_t fn_cmd,
                       const uint8_t *w_data, uint16_t w_len, uint16_t r_len) {
  assert_param(handle != NULL);
...
uint16_t ow_read_resp(ow_t *handle, uint8_t *data, uint16_t data_size) {
  ...
  if ((uint32_t)(handle->buf.write_len) + (uint32_t)len >
      sizeof(handle->buf.data)) {
    /* Truncate to available bytes */
    if (handle->buf.write_len < sizeof(handle->buf.data)) {
      len = (uint16_t)(sizeof(handle->buf.data) -
                       (uint32_t)handle->buf.write_len);
    } else {
      return 0;
    }
  }
  ...
  case 10:
    ...
    if (handle->search.val == OW_VAL_DIFF) {
      ...
      if (bit_number < handle->search.last_discrepancy) {
        ...
        bit_choice = (handle->search.rom_id[handle->buf.bit_idx / 8] >>
                      (handle->buf.bit_idx % 8)) &
                     0x01;
    ...
    if (handle->buf.bit_idx == 64) {
      ...
      if (ow_crc(handle->search.rom_id, 7) == handle->search.rom_id[7]) {
        ...
        if (handle->rom_id_filter == 0) {
          memcpy(&handle->rom_id[handle->rom_id_found], handle->search.rom_id,
                 8);
          handle->rom_id_found++;
        }
      ...
      if (handle->search.last_discrepancy == 0 ||
          handle->rom_id_found == OW_MAX_DEVICE) {
...
__STATIC_FORCEINLINE void ow_write_bit(ow_t *handle, bool high) {
  ...
  handle->config.gpio->BSRR =
      (high ? handle->config.pin_set : handle->config.pin_reset);
}
