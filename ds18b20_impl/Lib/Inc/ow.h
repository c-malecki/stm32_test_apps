...
/* Transfer a command and optional data to/from a specific 1-Wire device by SKIP
 * ROM */
ow_err_t ow_xfer(ow_t *handle, uint8_t fn_cmd, const uint8_t *w_data,
                 uint16_t w_len, uint16_t r_len);

#if (OW_MAX_DEVICE > 1)
/* Transfer a command and optional data to/from a specific 1-Wire device by ROM
 * ID index */
